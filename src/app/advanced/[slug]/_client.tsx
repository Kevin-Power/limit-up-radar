...
export default function AdvancedLessonClient({ slug }: { slug: string }) {
  const lesson = getAdvancedLessonBySlug(slug);
  const { prev, next } = getAdjacentAdvancedLessons(slug);
    ...
    return (
      <>
        ...
        <main id="main" className="max-w-3xl mx-auto px-4 py-20 text-center">
          ...
          <Link href="/advanced" className="mt-4 inline-block text-sm text-red hover:underline">
            &larr; 返回進階教室
          </Link>
        </main>
      </>
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-6 py-8 lg:flex lg:gap-8">
        ...
        <main id="main" className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 text-xs text-txt-4 mb-6">
            <Link href="/advanced" className="hover:text-txt-2 transition-colors">進階教室</Link>
            <span>/</span>
          ...
          {/* Per-lesson risk footnote */}
          <div className="mt-10 text-[11px] leading-relaxed text-txt-4 border-t border-border pt-4">
            本內容為教育與觀念說明，非投資建議、不構成買賣推薦，亦不保證任何報酬。選擇權與權證為高風險工具，交易前請充分了解商品規則與自身風險承受度。
          </div>

          {/* Navigation */}
          <div className="flex items-center justify-between mt-8 pt-6 border-t border-border">
            {prev ? (
              <Link href={`/advanced/${prev.slug}`} className="group flex flex-col">
            ...
            {next ? (
              <Link href={`/advanced/${next.slug}`} className="group flex flex-col text-right">
              ...
              <Link href="/advanced" className="group flex flex-col text-right">
        ...
        <aside className="hidden lg:block w-56 flex-shrink-0">
          <div className="sticky top-24">
            ...
            <div className="mt-6 pt-4 border-t border-border">
              ...
              <nav className="space-y-1">
                {ADVANCED_LESSONS.map((l) => (
                  <Link
                    key={l.slug}
                    href={`/advanced/${l.slug}`}
