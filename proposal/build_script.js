...
children.push(
  H1("講前須知"),
  H2("時間配比"),
  P("總長約 12 分鐘。封面 30 秒、痛點與平台介紹 2 分鐘、5 大功能各 45 秒（共 4 分鐘）、79% 勝率 hero 1 分鐘、競爭定位 1 分鐘、商業模式與財務 1.5 分鐘、行銷與路線圖 1.5 分鐘、風險與 CTA 30 秒。"),
  H2("語氣原則"),
  P("1. 不講「保證」「一定」「絕對」。講「目前數據顯示」「在這 99 個樣本中」。"),
  P("2. 數字要說全：不只說「79%」，要說「10 天 99 個樣本」。"),
  P("3. 競品要尊重：不貶低 CMoney 或 Goodinfo，他們有他們的優勢，我們是聚焦不同場景。"),
  P("4. 風險要主動講：不要讓對方問你才提，自己先說反而建立信任。"),
  H2("應對問題的口袋句"),
  P("Q「樣本太少？」→ 是的，目前只有 99 筆，這也是為什麼我們公開所有計算方法。樣本會持續累積，每天都在增加。"),
  P("Q「市場跌的時候呢？」→ 99 樣本裡涵蓋的是相對偏多頭的 10 天，這是限制。我們會在風險聲明裡明確揭露。"),
...
const slides = [
  {
    n: "01", title: "封面 · 79% 勝率 + URL", time: "30 秒",
    visual: "PPT 顯示巨大的「股文觀指 大師專區」品牌名，下方 79% 紅色大字。",
    script: "大家好。今天要介紹的是「股文觀指 大師專區」——一個聚焦台股漲停股的資訊平台。在開始講細節之前，先看一個數字。我們用 TWSE 真實隔日 OHLC、99 個樣本、10 個交易日、樣本加權平均做出來的隔日開盤勝率是 79%。這個數字後面我會仔細解釋是怎麼算出來的，現在請先記住這個 79%。",
    cue: { label: "停頓", text: "說完 79% 後停 2 秒，讓對方記住數字再繼續。" },
  ...
  },
  {
    ...
    transition: "現在進入今天最重要的一張投影片：那個 79% 是怎麼算出來的。",
  },
  {
    n: "09", title: "★ 79% 勝率 HERO", time: "70 秒",
    visual: "整張投影片只有一個巨大的「79%」紅色大字，下方三個小數字：78/99、+3.25%、10 天。",
    script: "這就是開頭講的 79%。我說明清楚是怎麼算的：對過去 10 個交易日，每天平台「明日焦點」推薦的所有標的，我們去抓真實的隔日 TWSE OHLC，看開盤價是不是高於漲停日收盤——如果是，算一次「贏」。10 天累積 99 個樣本，贏 78 次，勝率 79%。這是樣本加權平均，不是日均，所以一個 3 標的的日子和一個 30 標的的日子算法是公平的。平均開盤報酬是 +3.25%。但我必須誠實說：99 樣本是小樣本，10 天沒涵蓋空頭循環，這是限制。過去績效不代表未來。",
  ...
  },
  {
    ...
    script: "三層漏斗。最上層，SEO 經營「漲停股 隔日表現」「台股月營收 YoY」這類長尾關鍵字，每天自動更新的資料頁本身就是 SEO 內容，自然會被搜尋引擎收錄。中層，靠真實數據揭露建立信任——79% 勝率、99 樣本、開源演算法，這是我們和「老師喊單」最大的區別。下層，免費 LINE 群作為高互動社群，使用者實際看到價值之後自然轉成月費會員。整條漏斗不用大量廣告投放，是時間累積換流量。",
    transition: "時間軸具體看一下。",
  },
  {
  ...
  },
  {
    ...
    script: "風險我必須直接講清楚。第一，樣本只有 99 筆 10 天，沒有跨多空頭循環，未來市況差會有顯著差異。第二，這是個人開發專案，目前沒有團隊和客服窗口。第三，我們不是金融顧問業者，所有內容都僅供參考，不構成任何投資建議。第四，每日資料盤後 2.5 小時才更新，盤中即時請看券商。第五，漲停股操作是高波動策略，可能虧損，請在自己能承受的範圍操作。我把這些先講清楚，是因為信任建立在不掩蓋限制之上。",
...
children.push(
  ...
  H2("延伸場景變奏版"),
  P("【3 分鐘 elevator pitch 版】只用 Slide 1 + Slide 9 + Slide 11 + Slide 16。一張封面、一張 79% 勝率、一張商業模式、一張 CTA。適合電梯、餐會、KOL 訪談開場。"),
