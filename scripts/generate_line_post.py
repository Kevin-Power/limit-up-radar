...
def build_public_text(d, picks, next_day):
    ...
    # 開頭：價值主張（分析/教育定位）+ 個人紀錄聲明 + 誠實統計錨
    text = (
        "📡 漲停雷達\n"
        "每天盤後，幫你把今天漲停的股票分好族群、整理隔日行為統計與觀察筆記，免費。\n"
        "（個人交易紀錄與資料整理分享，非投顧、未收費，不構成個股推薦）\n"
        "━━━━━━━━━━━━━━━━━━\n"
        f"📊 統計觀察：回測 {bt['totalSamples']} 樣本（偏多頭區間・未含交易成本）\n"
        f"   隔日開盤勝率 {bt['avgOpenWinRate']}%・平均 +{bt['avgOpenReturn']}%（毛報酬，僅供研究）\n"
        "━━━━━━━━━━━━━━━━━━\n"
    )
...
過去回測（{bt['totalSamples']} 樣本、偏多頭區間、未含交易成本與滑價）不代表未來績效。
